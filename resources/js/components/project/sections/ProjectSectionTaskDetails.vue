                ...
                <div class="project_slide_details_header d-flex align-items-center">
                    ...
                    <div class="btn-group square_arrow_down_btn square_btn_32px ">
                        ...
                        <div
                            ...
                            <a href="#" @click.prevent="taskDelete" class="dropdown-item"> <i class="far fa-trash filter_icon mr-2"></i>{{ trans('project::project.delete_task') }}</a>
                        </div>
                    </div>
                    <div class="btn-group square_arrow_down_btn square_btn_32px ">
                ...
                <div class="project_d_body pb_30">
                    ...
                    <div class="task_d_filed">
                        <template v-for="(field, field_index) in task.all_fields">
                            <project-section-task-details-fields :field="field" :project="project" :field_index="field_index" :task="task" :key="field_index+1" ref="project_field_details" @updateTaskFieldValue="updateTaskFieldValue"/>
                        </template>
                    </div>
                    ...
                    <div class="project_d_thumbs mt_50">
                        <file-upload-input @showImage="AddClassForImage" :button-text="'Upload document'" :token="taskForm.upload_token" module="task" :clear-file="clearAttachment" :module-id="task.id" ref="file_upload_input" @fileUploaded="getTask"></file-upload-input>
                    </div>
                    <div class="project_d_noteLists">
                        ...
                        <div class="project_d_noteLists_header mb_10" v-for="(comment, index) in task.pined_comments"
                                    ...
                                    <span class="f_s_13 f_w_400 black_text mb-0 line_h_1">
                                        ...
                                        <span> {{ comment.event? comment.event.replace('_', ' ') : '' }} </span>
                                            <template v-if="comment.event == 'assigned_to'">
                                                    <span v-if="comment.new_id == auth_user.id">You</span>
                                            ...
                                            <template v-else-if="(comment.event == 'update_field')">
                                                    {{ comment.field.name }} to "{{ comment.comment }}"
                                            </template>
                            ...
                            <div class="thumb_big ml_42" v-if="comment.event == 'attached'">
                                <a class="AttachmentImthumb_big_preview" @click.prevent="showCommentImage(comment.new.id)">
...
import ProjectSectionTaskDetailsSubTasks from './subtask/ProjectSectionTaskDetailsSubTasks'
...
export default {
    ...
    methods: {
        ...
        updateName(event){
            this.input_changed=true;
            this.$emit('updateField', 'name', event.target.value);
        },
        ...
        completeMarkTask() {
            ...
            this.$emit('updateField', 'completed', 1);
        ...
        incompleteMarkTask() {
            ...
            let task_id = this.task.id;
            this.$emit('updateField', 'completed', 0);
            axios.post("/task-complete",{
                task_id,
                value : 0
            }).then(res => {
        ...
        showCommentImage(image_id){
            this.$nextTick(() => {
                this.$refs.file_upload_input.showImageBy(image_id);
            }); 
        },
        handleTaskChange() {
