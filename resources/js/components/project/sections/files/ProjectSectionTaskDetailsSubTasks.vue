    ...
    <div class="task_group_list details_subtask ">
        <div class="task_seperate_single_list d-flex bordered_header sortitem">
            ...
            <label class="task_name mb-0" for="#">
             ...
             <span class="chat_icon" v-if="task.comments.length">
                <i class="ti-comment"></i> {{ task.comments.length }}
            </span>
            ...
            <template v-for="(field, field_index) in task.fields">
                <project-section-sub-task-fields :field="field" :field_index="field_index" :task="task" :key="field_index+1" :project="project"/>
            </template>
...
import ProjectSectionSubTaskFields from '../subtask/ProjectSectionSubTaskFields'
export default {
    name: "project-section-task-details-sub-tasks",
    components : {
        draggable,  ProjectSectionSubTaskFields
    },
    props: {
        task : {
            type : Object
    ...
    methods: {
        openSubtaskDetails(){
            this.$emit('showTask', this.task.uuid);
        },
        addTask(){
        ...
        submitTaskName() {
            if (this.input_changed) {
                if (typeof (this.task) != 'undefined' && !this.task.name) {
                   this.$emit('taskDeleted', this.task_index, false)
                }
