...
class IncomeStatementReportRepository implements IncomeStatementReportRepositoryInterface
{
    public function search($timePeriod)
    {
        ...
        if ($accountingPeriod->end_date) {
            ...
            // return $acc_id = Transaction::where('account_id', 15)->whereBetween('created_at' , array($accountingPeriod->start_date." 00:00:00", $accountingPeriod->end_date." 23:59:59"))->sum('amount');
        }else {
            return Transaction::whereHasMorph('voucherable','*', function(Builder $query) use($accountingPeriod){
                $query->where('is_approve', 1)->whereBetween('date' , array($accountingPeriod->start_date, Carbon::now()->toDateString()));
            })->sum('amount');
            // $acc_id = Transaction::where('account_id', 15)->whereBetween('created_at' , array($accountingPeriod->start_date." 00:00:00", Carbon::now()))->sum('amount');
        }
    }
    ...
    public function saleTransactionBalance($timePeriod)
    {
        ...
        if ($accountingPeriod->end_date) {
            ...
            // Transaction::where('account_id', 15)->whereBetween('created_at' , array($accountingPeriod->start_date." 00:00:00", $accountingPeriod->end_date." 23:59:59"))->sum('amount');
        }else {
            ...
            // Transaction::where('account_id', 15)->whereBetween('created_at' , array($accountingPeriod->start_date." 00:00:00", Carbon::now()))->sum('amount');
        }
