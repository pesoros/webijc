                    ...
                    <div class="white_box_50px box_shadow_white pb-3">
                        <form class="" action="{{ route('sales_return_report.index') }}" method="GET">
                            <div class="row">
                                ...
                                <div class="col">
                                    <div class="primary_input mb-15">
                                        <label class="primary_input_label" for="">{{ __('report.Retailer') }}</label>
                                        <select class="primary_select mb-15" name="retailer_id" id="retailer_id">
                                            <option value="">{{__('attendance.Choose One')}}</option>
                                            @isset($retailer_id)
                                                @foreach ($retailers as $retailer)
                                                    <option value="{{ $retailer->id }}" @if ($retailer->id == $retailer_id) selected @endif>{{ $retailer->name }}</option>
                                                @endforeach
                                            @else
                                                @foreach ($retailers as $retailer)
