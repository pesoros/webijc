            ...
            <div class="row justify-content-center">
                ...
                <div class="col-lg-12 mb-3">
                    <div class="white_box_50px box_shadow_white pb-3">
                        <form class="" action="{{ route('customer.bill.search') }}" method="GET">
                            <div class="row">
                                    ...
                                    <div class="primary_input mb-15">
                                        ...
                                        <select class="primary_select mb-15" name="customer_id" id="customer_id">
                                            ...
                                            @foreach ($customers as $customer)
                                                <option
                                                    value="{{ $customer->id }}" {{ isset($customer_id) && $customer->id == $customer_id ? 'selected' : '' }}>{{ $customer->name }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row justify-content-center">
                                ...
                                <div class="primary_input ml-2">
                                    <a href="{{route('customer.bill')}}" class="primary-btn fix-gr-bg"
                ...
                @isset($accounts)
                    <div class="col-12">
                        <div class="box_header common_table_header">
                            <div class="main-title d-md-flex">
                                <h3 class="mb-0 mr-30 mb_xs_15px mb_sm_20px">{{ __('report.Billing Report') }}</h3>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12">
                                ...
                                <div class="">
                                    <table class="table Crm_table_active3">
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col">{{__('purchase.Payment Term')}}</th>
                                        ...
                                        @endphp
                                            @foreach($accounts as $key=> $account)
                                                <tr>
                                                    <td>{{$key+1}}</td>
                                                    <td>{{date(app('general_setting')->dateFormat->format, strtotime($key == 'sales' ? $account->date : $account->created_at))}}</td>
                                                    <td>{{$account->invoice_no}}</td>
                                                    <td>{{$account->ref_no}}</td>
                                                    <td>{{$account->customer->pay_term.' '}} {{$account->customer->pay_term_condition}}</td>
                                                    @php
                                                        $total_amount += $account->payable_amount;
                                                        $given_amount += $account->billing_amount['paid_amount'];
                                                        $total_due += $account->billing_amount['due_amount'];
                                                    @endphp
                                                    <td>{{single_price($account->payable_amount)}}</td>
                                                    <td>{{single_price($account->billing_amount['paid_amount'])}}</td>
                                                    <td>{{single_price($account->billing_amount['due_amount'])}}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                        <tfoot>
                                        <tr>
                                            <td colspan="5"><b>{{__('report.Total')}} :</b></td>
