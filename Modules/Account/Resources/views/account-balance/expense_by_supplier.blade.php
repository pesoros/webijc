...
@extends('backEnd.master', ['title' => 'Expense by supplier'])
@section('mainContent')
            ...
            <div class="row justify-content-center">
                ...
                <div class="col-lg-12 mb-3">
                    <div class="white_box_50px box_shadow_white">
                        <form class="" action="{{ route('expense_by_supplier') }}" method="GET">
                            <div class="row">
                    ...
                    <div class="col-12 mt-5">
                        <div class="box_header common_table_header">
                            <div class="main-title d-md-flex">
                                <h3 class="mb-0 mr-30 mb_xs_15px mb_sm_20px">{{__('account.Expense By Supplier')}}</h3>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="QA_table ">
                                ...
                                <div class="">
                                    <table class="table Crm_table_active3">
                                        ...
                                        <tr>
                                            <th>#</th>
                                            <th scope="col">{{ __('account.Supplier') }}</th>
                                            <th scope="col" class="text-right">{{ __('account.Expense') }}</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @php
                                            $total = 0;
                                        @endphp
                                        @foreach($chartAccount as $account)
                                            @php
                                                 if($dateFrom==null && $dateTo==null) {
                                                    $transactions = $account->Debit;
                                                }else{
                                                    $transactions = $account->transactions()->whereBetween('created_at',[$dateFrom, $dateTo])->debit;
                                                }
                                            $total += $transactions;
                                            @endphp
                                            <tr>
                                                <td>{{ $loop->index + 1 }}</td>
                                                <td>
                                                    <a href="{{ route('supplier.view', $account->contactable_id) }}" target="_blank">{{ $account->name }}</a>
                                                </td>
                                                <td class="text-right">{{ single_price($transactions) }}</td>
                                            </tr>
